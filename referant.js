...
let draggedNodePathInfo = undefined;
function dropDraggedNode(targetFolder) {
...
function makeFolderElement(folderPath, i, folder, callback, closeCallback) {
...
function makeLayoutElement(folderPath, i, layout, callback, closeCallback) {
    let $div = $(document.createElement('div'));
    $div.addClass('folder');
    $div.addClass('folder-draggable-node');
    let $img = $(document.createElement('img'));
...
let activated = "none";
const filters = ['color', 'view', 'license'];
...
const colorFilterNames = ["red", "yellow", "blue", "green", "purple", "orange", "white", "gray", "black"];
const viewFilterNames = ["front", "side", "back"];
const licenseFilterNames = ["reuse", "public domain"];
let activeFilters = new Map();
function makeColorCheck(color) {
    const ret = $('<span>', {
...
function makeRadioButton(name, label, callback, checked) {
    const id = name + '-' + label;
    const $ret = $('<span>');
    const $button = $('<input>', {
        'type': 'radio',
...
function makeCheckbox(name, label, callback, checked, disabled) {
    const id = name + '-' + label;
    const $ret = $('<span>');
    const $button = $('<input>', {
        'type': 'checkbox',
...
function matchesQuery(image, query0) {
    const query = query0.toLowerCase();
    if (image.filename.indexOf(query) !== -1)
        return true;
...
function matchesFilter(image, filter) {
    console.log(filter);
    let values = image[filter.key];
    if (values === undefined)
        return false;
...
let addViewerShown = true;
const DEFAULT_RATIO = 0.1;
function rerenderFilesystem() {
    rerenderFilesystemViewer(filesystemPath, $('#main-folders'));
    rerenderFilesystemViewer(addViewerPath, $('#add-viewer-outer'), "You don't have any images in this folder you can add! Go to Folders to find and add an image first.", (image) => {
        addImage({
            x: 0,
            y: 0,
            ratio: DEFAULT_RATIO,
    ...
    rerenderFilesystemViewer(searchAddPath, $('#search-add-modal'));
    rerenderFilesystemViewer(saveLayoutPath, $('#save-layout-modal'));
}
function rerenderFilesystemViewer(path, $parent, emptyMsg, imageCallback, layoutCallback) {
    renderPath(path, $parent.find('.fs-path'));
    renderFiles(path, $parent.find('.fs-viewer'), emptyMsg, imageCallback, layoutCallback);
}
...
function fillErrorModal(folderName) {
    $('span.error-modal-contents').each((_, thing) => {
        console.log(thing);
        $(thing).text(folderName);
    });
}
function attachToErrorModal(callback) {
...
function renderFiles(path, $target, emptyMsg, imageCallback, layoutCallback) {
...
let focusedSearchImage = undefined;
...
function updateSearchResults() {
    ...
    /*if(query !== '' || allActiveFilters.length !== 0)*/ for (const image of images) {
        if (matchesQuery(image, query.toString()) &&
            allActiveFilters.every((filter) => matchesFilter(image, filter))) {
            const $img = $('<img/>', {
                src: image.filename,
            });
            $img.click(() => {
...
let viewFilterOn = false;
let viewFilter = new Set(viewFilterNames);
let licenseFilter = undefined;
function clearColorFilter() {
    colorFilter.clear();
    $('#color-activate').text('Color: any');
    updateSearchResults();
}
function toggleColorFilter(filter) {
    if (colorFilter.has(filter))
        colorFilter.delete(filter);
    else
    ...
    if (colorFilter.size == 0) {
        ...
        let name = "error";
        for (const thing of colorFilter)
            name = thing;
...
function toggleViewFilter(filter) {
    if (viewFilter.has(filter))
        viewFilter.delete(filter);
    else
...
function serializedCopy(layoutInfo) {
...
function dragCompute(info, event) {
    if (!dragging) {
    ...
    let x = info.x;
    let y = info.y;
    let rawImg = info.element.find('img')[0];
    ...
    let negX; // are we dragging one of the west corners?
    let negY; // are we dragging one of the north corners?
    if (dragging.type === 'nw') {
        [negX, negY] = [true, true];
    }
    ...
    if (negX)
        avgRatio = Math.min(avgRatio, (x + w) / nw);
    if (negY)
        avgRatio = Math.min(avgRatio, (y + h) / nh);
...
$(document).mousemove((event) => {
    ...
    if (dragging.type === 'move') {
        info.element.css('transform', `translate(${Math.max(0, info.x + event.clientX - dragging.x)}px, ${Math.max(0, info.y + event.clientY - dragging.y)}px)`);
    }
    else {
...
$(document).ready(() => {
    ...
    $('#add-to-folders').click(() => {
        if (focusedSearchImage) {
            $('#search-add-modal img.focused').attr('src', focusedSearchImage.filename);
            $('#search-add-modal .filename').text(focusedSearchImage.filename);
